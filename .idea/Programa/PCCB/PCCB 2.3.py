...
class DatosSecciones(wx.Frame):
    def __init__(self, parent):
        ...
        self.self.panel = wx.self.panel(self, -1)
        scrolled.Scrolledself.panel(self.panel, style=wx.VSCROLL)
...
class AsignacionSecciones(wx.Frame):
    def __init__(self, parent):
        ...
        wx.StaticLine(panel, -1, (5, 5), (400, 2))
        dv_st = wx.StaticText(panel, label="ASIGNAR SECCIONES", pos=(100, 10))
        wx.StaticLine(panel, -1, (5, 30), (400, 2))

        dv_st.SetFont(wx.Font(12, wx.SWISS, wx.NORMAL, wx.BOLD))
        wx.StaticLine(panel)
        wx.StaticText(panel, label="VANO N:", pos=(10, 45))
        wx.StaticText(panel, label="N SECCION", pos=(80, 45))
        wx.StaticText(panel, label="SECCION \n RESISTENTE", pos=(165, 45))
        wx.StaticText(panel, label="AREA \n [m2]", pos=(255, 45))
        wx.StaticText(panel, label="INERCIA \n [m4]", pos=(310, 45))
        wx.StaticLine(panel, -1, (5, 85), (400, 2))
        ...
        # Si tengo voladizo izquierdo...
        if datos_dg[2] == 1:
            finvanos = datos_dg[0]*10 + 11
            for t in range(self.filas):
                if t < 10:
                    j = 30
                    wx.StaticText(panel, label=str('Vol. Izq.'), pos=(12, l))
                    j += 30
                    nsec = wx.StaticText(panel, label=str(t+1), pos=(j, l))
                    j += 50
                    self.secin = wx.TextCtrl(panel, value = '', pos=(j, l - 5), size=(80, -1))
                    l += 50
                else:
                    if t >= 10 and t < finvanos:
                        if t % 10 == 0:
                            wx.StaticText(panel, label=str('Apoyo'), pos=(12, l))
                        else:
                            wx.StaticText(panel, label=str('Vano ' + str(int((t-10)/10 + 2))), pos=(12, l))
                        j = 110
                        nsec = wx.StaticText(panel, label=str(t+1), pos=(j, l))
                        j += 50
                        self.secin = wx.TextCtrl(panel, value = '', pos=(j, l - 5), size=(80, -1))
                    ...
                    # Si tiene voladizo derecho...
                        ...
                        if datos_dg[3] == 1:
                            j = 30
                            wx.StaticText(panel, label=str('Vol. Der.'), pos=(12, l))
                            j += 30
                            nsec = wx.StaticText(panel, label=str(t+1), pos=(j, l))
                            j += 50
                            self.secin = wx.TextCtrl(panel, value = '', pos=(j, l - 5), size=(80, -1))
        ...
        self.scroll = scrolled.ScrolledPanel(panel)
